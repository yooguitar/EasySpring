...
public class MemberServiceImpl implements MemberService {
	 ...
	 * 2. 임의값(문자열 4자리 수 랜덤)생성 후 MailDTO 객체 생성하여 adminService.mailForUser(메일 전송 모듈)에 전달
	 * 3_1. HttpSession 사용을 위해 필드 주입 
	 * 3_2. 세션에 userId(Key), randomNum(Value) 저장 => 사용자가 메일 확인 후 입력하는 randomNum을 userId 키값으로 식별하기 위함
	 * 4. response 200ok => 다음 단계 이동 setIsSuccess(1) 
	 * 
	 * 		임시 저장소로 static 클래스 하나 만들어서 거기에 HashTable > Map<String, String> 형태로 모아뒀다가 꺼내서 쓰는 아이디어?
	 * 		=> IOC 원칙 위배 + 구현 실패
	 * 		==> 임시 저장소 세션 선택
	 * 
	 * 2단계
	 *  matchRandomNum():
	 *  1. getAttribute(userId) => 앞단에서 localStorage 사용자 userId 값을 같이 넘겼음. 이를 키 값 으로 세션에서 randomNum 찾기
	 *  2. 일단 여기서 세션 찾으면 null 입니다.	
	 *  3. 사용자 입력 randomNum과 세션value(randomNum) 비교하여 다르면 예외 발생
	 *  4. response 200ok => 다음 단계 이동 setIsSuccess(2)
	 *  
	 *  3단계
	 *  1. 변경할 비밀번호 입력 받고 인코딩하여 저장 => Valid 통과할 경우 다른 예외는 발생 X
	 *  2. response 200ok => 변경 완료
	 *  
	 */
	@Override
	public void findPwd(Map<String, String> request) {
        ...
        session.setAttribute(userId, randomNum);
        System.out.println("set세션 밸류:" + session.getAttribute(userId));
	...
	public void matchRandomNum(Map<String, String> request) {
		...
		System.out.println("찾은 세션값:" + session.getAttribute(userId));
		//String sessions = (String)session.getAttribute(userId); // 사용자 요청 Id와 세션의 키값이 같은걸 찾음
